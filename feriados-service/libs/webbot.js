...
if (!year) {
    ...
    try {
        return await page.evaluate((year)=> {
            ...
            const monthsDiv = [...calendarContainer.querySelectorAll(":scope > div[id^='m']")];

            //const holidaysDiv = calendarContainer.querySelectorAll('div.holidays')
            //if(holidaysDiv.length !== 11) throw new Error('holidays monthly div differes from expected value')

            monthsDiv.forEach( (ccc, index) => {

                const month = String(index + 1)

                const holidaysDiv = ccc.querySelector('div.holidays')

                if(holidaysDiv){

                    holidaysDiv.querySelectorAll('p').forEach(p => {
